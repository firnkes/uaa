...
public class AuthzAuthenticationManager implements AuthenticationManager, ApplicationEventPublisherAware {
    ...
    public AuthzAuthenticationManager(UaaUserDatabase userDatabase, PasswordEncoder encoder, IdentityProviderProvisioning providerProvisioning, IdentityZoneManager identityZoneManager) {
    ...
    public Authentication authenticate(Authentication req) throws AuthenticationException {
        ...
        if (user == null) {
            ...
            if (!passwordMatches) {
                ...
                publish(new IdentityProviderAuthenticationFailureEvent(req, req.getName(), OriginKeys.UAA, identityZoneManager.getCurrentIdentityZoneId()));
                publish(new UserAuthenticationFailureEvent(user, req, identityZoneManager.getCurrentIdentityZoneId()));
            } else {
                ...
                if (!(allowUnverifiedUsers && user.isLegacyVerificationBehavior()) && !user.isVerified()) {
                    publish(new UnverifiedUserAuthenticationEvent(user, req, identityZoneManager.getCurrentIdentityZoneId()));
                ...
                publish(new IdentityProviderAuthenticationSuccessEvent(user, success, OriginKeys.UAA, identityZoneManager.getCurrentIdentityZoneId()));
                return success;
            }
        }
    ...
    protected int getPasswordExpiresInMonths() {
        int result = 0;
        IdentityProvider provider = providerProvisioning.retrieveByOriginIgnoreActiveFlag(OriginKeys.UAA, identityZoneManager.getCurrentIdentityZone().getId());
        if (provider!=null) {
