...
public class ChangeEmailEndpoints implements ApplicationEventPublisherAware {
    ...
    public ResponseEntity<String> generateEmailVerificationCode(@RequestBody EmailChange emailChange) {
        ...
        if (user.getUserName().equals(user.getPrimaryEmail())) {
            List<ScimUser> results = scimUserProvisioning.query("userName eq \"" + email + "\" and origin eq \"" + OriginKeys.UAA + "\"", identityZoneManager.getCurrentIdentityZone().getId());
            if (!results.isEmpty()) {
        ...
        String code;
        try {
            code = expiringCodeStore.generateCode(JsonUtils.writeValueAsString(emailChange), new Timestamp(System.currentTimeMillis() + EMAIL_CHANGE_LIFETIME), EMAIL.name(), identityZoneManager.getCurrentIdentityZone().getId()).getCode();
    ...
    public ResponseEntity<EmailChangeResponse> changeEmail(@RequestBody String code) throws IOException {
        ExpiringCode expiringCode = expiringCodeStore.retrieveCode(code, identityZoneManager.getCurrentIdentityZone().getId());
        if ((null != expiringCode) && ((null == expiringCode.getIntent()) || EMAIL.name().equals(expiringCode.getIntent()))) {
            ...
            scimUserProvisioning.update(userId, user, identityZoneManager.getCurrentIdentityZone().getId());

            String redirectLocation = null;
            String clientId = data.get("client_id");

            if (clientId != null && !clientId.equals("")) {
                ClientDetails clientDetails = clientDetailsService.retrieve(clientId, identityZoneManager.getCurrentIdentityZone().getId());
                redirectLocation = (String) clientDetails.getAdditionalInformation().get(CHANGE_EMAIL_REDIRECT_URL);
            }

            publisher.publishEvent(UserModifiedEvent.emailChanged(userId, user.getUserName(), user.getPrimaryEmail(), identityZoneManager.getCurrentIdentityZoneId()));
